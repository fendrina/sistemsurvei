...
{
    public function get_data($prodi_mhsw)
	{
      return $this->db->table('prodi')
      ->where(array('kode_prodi' => $prodi_mhsw))
      ->get()->getRowArray();
	}
    ...
    public function masukMhsw($data)
    {
        $this->db->table('tb_dosenmhsw')->insert($data);
    }

    public function getMhswProdi()
    {
    ...
    {
        $result = array();
        $kode_prod = $this->getMhswProdi()->prodi;
        // dd($kode_prod);
                ...
                foreach($params['id_jadwal'] as $key3 => $value3) {
                    if ( $key == ((int) $key2 + 2) && $key2 == $key3) {
                        $data = [
                            ...
                            'role_user'=> 2, //role dosen 1, mhsw 2, tendik 3
                            'id_jadwal' => $value3
                        ];
