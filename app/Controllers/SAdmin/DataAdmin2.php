...
class DataAdmin2 extends BaseController
{
     //Data admin sarana dan prasarana
    protected $da2Model; //OOP
    public function __construct()
    {
        helper('form');
        $this->da2Model = new AdminUnit_M();
        $this->auth = new \IonAuth\Libraries\IonAuth();
        $this->user = $this->auth->user()->row();
        $this->group_name = $this->auth->getUsersGroups()->getRow()->name;
        $this->group_id = $this->auth->getNameGroup('unit2')->id;
    }
    public function index()
    {
        $data = [
            'title'      => 'Data Admin Unit Sarana dan Prasarana',
            'user'       => $this->user,
            'group_name' => $this->group_name,
            'admin'      => $this->da2Model->getAdmin2()
        ];
       
        
        return view('SAdmin/adminUnit/dataAdmin2', $data);
    ...
    public function save()
    {
        ...
        return redirect()->to(base_url('SAdmin/dataAdmin2'));
    }
    ...
    public function delete($id){
        ...
        $this->da2Model->deleteAdmin2($data);
